...
function update_editor(prg)
{
    ...
    sub_strings = raw_string.split(/\(\*\)/g);

    for (index = 0; index < sub_strings.length; index++)
    {
        if ((index % 2) == 1)
            string+= compatible_characters(sub_strings[index], 1);
        else
            string+= compatible_characters(sub_strings[index], 0);
...
function insert_ch(prg, id)
{
    ...
    editor.selectionStart = id.length + selection_end;
    editor.selectionEnd = id.length + selection_end;
    editor.focus();
